...
void MyFrame2::OnVirement(wxCommandEvent& event) {
    ...
    if (new_account->ShowModal() == wxID_OK)
    {
        ...
        //auto account_numbers = rand(); // Only one at this time
        //auto Lombre_compte = std::to_string(account_numbers);
        ////wxMessageBox(wxT("Votre num�ro de compte est le suivant : ") + wxT(ombre_compte));
        ...
        try
        {
            ...
            if ((verif_account_exists(pt_write, compte_emetteur) == 1) && (verif_account_exists(pt_write, compte_recepteur) == 1)) {
                ...
                pt_write = edit_solde_of_an_account(pt_write, compte_emetteur, -compte_montant); // On soustrait � l'�metteur le montant d'o� le signe - 
                pt_write = edit_solde_of_an_account(pt_write, compte_recepteur, compte_montant); // On ajoute au recepteur le montant

                //On ajoute ces op�rations dans chacun des comptes
                ...
                if (verif_account_exists(pt_write, compte_emetteur) == 1) {
                    ...
                    bankjson2 = ClientRequest("getnbbank|" + to_string(compte_recepteur));


                    bankjson2 = ClientRequest("getbank|" + bankjson2);




                    ptree pt_write2;
                    std::ifstream file_in(bankjson2);
                    read_json(file_in, pt_write2);
                    file_in.close();

                    pt_write = edit_solde_of_an_account(pt_write, compte_emetteur, -compte_montant); // On soustrait � l'�metteur le montant d'o� le signe - 
                    pt_write2 = edit_solde_of_an_account(pt_write2, compte_recepteur, compte_montant); // On ajoute au recepteur le montant
                    ...
                    pt_write = write_an_operation(pt_write, operation1, get_an_account(pt_write, compte_emetteur));
                    pt_write2 = write_an_operation(pt_write2, operation2, get_an_account(pt_write2, compte_recepteur));

                    std::ofstream file_out2(bankjson);
                    write_json(file_out2, pt_write);
                    file_out2.close();

                    std::ofstream file_out3(bankjson2);
                    write_json(file_out3, pt_write2);
                    file_out3.close();
